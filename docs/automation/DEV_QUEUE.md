...
## 사용법
이 문서는 현재 GPT가 어디까지 작업해도 되는지를 고정한다.
...
GPT는 `ACTIVE` 또는 사용자가 명시적으로 승인한 범위를 넘어 개발하지 않는다.
...
그 범위 안에서는 `PROJECT_RULES.md`의 AUTO 항목을 GPT가 스스로 판단한다.

---

# Decision Gate
작업 중 아래 상황이 나오면 상태를 `BLOCKED`로 바꾸고 사용자에게 보고한다.
- 핵심 루프 변경 필요
